...
async function tooBig(toolbox, askPrompt, res, needAsk) {
    ...
    if (confirmation) {
        ...
        const a = (0, child_process_1.exec)(type);
        a.stdout.on("data", (a) => {
            console.log(a);
        });
        a.on("close", (a) => {
        ...
        console.log(toolbox.print.colors.muted(res.added.logPath.replace(`\\`, "/")));
    }
    else {
        console.log(toolbox.print.colors.muted(askPrompt.Project.toUpperCase() + " LOGS ---------"));
        console.log(res.added.translated);
        console.log(toolbox.print.colors.muted(askPrompt.Project.toUpperCase() + " LOGS END ---------"));
        process.kill(0);
    }
}
            ...
            setTimeout(async () => {
                index_1.NodeCloudApi.api.post
                    ...
                    if (res.data.ok) {
                        ...
                        if ((args[2] != null && args[2].toLowerCase() == "--open") ||
