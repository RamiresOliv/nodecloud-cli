    ...
    setTimeout(async () => {
        ...
        index_1.NodeCloudApi.api.post.bin
            ...
            const askProjects = {
                type: "select",
                name: "Project",
                message: "Qual o seu projeto que você deseja ver informações mais detalhadas?",
            ...
            index_1.NodeCloudApi.api.post.bin
                ...
                if (!res || res.data.ok) {
                    ...
                    toolbox.print.info(toolbox.print.colors.muted(res.data.return.projectInfos.description || "Cool App"));
                    let status = toolbox.print.colors.red("OFFLINE");
                    if (res.data.return.projectInfos.status
                        .toLowerCase()
                        .includes("online"))
                        status = toolbox.print.colors.green("ONLINE");
                    toolbox.print.info("Stado: " + status);
                    toolbox.print.info("Linguagem: " +
                        toolbox.print.colors.muted(res.data.return.configFile.LANGUAGE));
                    toolbox.print.info("Versão: " +
                        toolbox.print.colors.muted("v" + res.data.return.configFile.VERSION));
                    toolbox.print.info("Starter: " +
                        toolbox.print.colors.muted(res.data.return.configFile.START));
                    toolbox.print.info("Tamanho: " +
                        toolbox.print.colors.muted(`${res.data.return.projectInfos.size}`));
                    toolbox.print.info(toolbox.print.colors.muted('Essas informações são do Docker e do arquivo "cloud.config" de sua aplicação.'));
                    toolbox.print.info("-".repeat(100));
                    process.kill(0);
                }
                else {
                    if (res.data.errcode == 500) {
                        ...
                        spinner1.fail(toolbox.print.colors.red(res.data.message +
                            toolbox.print.colors.muted(" ☁️ Tente novamente mais tarde! Desculpe :<")));
                        process.kill(0);
                    }
                }
